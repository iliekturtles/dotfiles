...
{{#if dotter.packages.games}}{{#if games.poe.awakened}}AddPackage --foreign awakened-poe-trade-git # Path of Exile trading app for price checking{{/if}}{{/if}}
...
{{#if dotter.packages.rust}}AddPackage --foreign cargo-sweep # A cargo subcommand for cleaning up unused build files generated by Cargo{{/if}}
